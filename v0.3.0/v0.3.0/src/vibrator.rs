...
impl Allpass {
    ...
    fn process(&mut self, input: f32) -> f32 {
        let read_pos = (self.write_pos + ALLPASS_SIZE - self.delay) % ALLPASS_SIZE;
...
mod tests {
    ...
    fn impulse_produces_decaying_pitched_signal() {
        ...
        assert!(
            energy > 0.001,
            "Impulse should excite the string, got RMS {energy}"
        );
    ...
    fn brightness_affects_spectral_content() {
        ...
        let hf_bright: f32 = out_bright.windows(2).map(|w| (w[1] - w[0]).abs()).sum();
    ...
    fn position_changes_timbre() {
        ...
        let hf_bridge: f32 = out_bridge.windows(2).map(|w| (w[1] - w[0]).abs()).sum();
    ...
    fn reset_silences() {
        ...
        assert!(peak(&out) < 1e-6, "After reset, string should be silent");
    }
