...
impl MothVoice {
    ...
    /// Process with optional external audio replacing/blending the exciter.
    ///
    /// When `external` is `Some`, the external audio is crossfaded with the
    /// internal exciter output before feeding the vibrator:
    ///
    /// ```text
    /// exciter ──┐
    ///           ├── crossfade(ext_mix) ──→ vibrator ──→ body ──→ ...
    /// external ─┘
    /// ```
    ///
    /// `ext_mix = 0.0` → internal exciter only (same as `process()`).
    /// `ext_mix = 1.0` → external audio only.
    /// `ext_mix = 0.5` → equal blend.
    ///
    /// The external audio is resonated by the waveguide at the current
    /// pitch, coloured by the body, and shaped by non-lin and spatial —
    /// as if you were blowing/striking the instrument with the external
    /// signal as the excitation source.
    pub fn process_with_external(
        &mut self,
        model: &ExciterModel,
        gesture: &PlayGesture,
        external: Option<&[f32]>,
        ...
        // ── External input crossfade ──
        if let Some(ext) = external {
            if ext_mix > 0.001 {
                let int_gain = 1.0 - ext_mix;
