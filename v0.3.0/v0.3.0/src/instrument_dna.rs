...
mod tests {
    ...
    fn differentiation() {
        ...
        assert_ne!(
            a.exciter.signature, b.exciter.signature,
            "Adjacent seeds must produce different signatures"
        );
        assert_ne!(
            a.resonator.stereo_offset, b.resonator.stereo_offset,
    ...
    fn all_values_in_sweet_spot_ranges() {
        ...
        let seeds = [
            ...
            0xCAFE_BABE,
            0x1234_5678,
            0xF0CA_CC1A, // the focaccia constant itself
        ];
        ...
        for seed in seeds {
            ...
            // Exciter ranges
            assert!(
                (0.0..1.0).contains(&dna.exciter.signature),
                "seed 0x{seed:08X}: signature {:.4} out of [0,1)",
                dna.exciter.signature
            );
            assert!(
                (0.0..1.0).contains(&dna.exciter.noise_phase_offset),
                "seed 0x{seed:08X}: noise_phase {:.4} out of [0,1)",
                dna.exciter.noise_phase_offset
            );
            assert!(
                (0.42..=0.58).contains(&dna.exciter.stochastic_bias),
                "seed 0x{seed:08X}: stochastic_bias {:.4} out of [0.42,0.58]",
                dna.exciter.stochastic_bias
            );
            assert!(
                (0.88..=1.02).contains(&dna.exciter.spectral_tilt_bias),
                "seed 0x{seed:08X}: spectral_tilt_bias {:.4} out of [0.88,1.02]",
                dna.exciter.spectral_tilt_bias
            );
            assert!(
                (0.85..=1.15).contains(&dna.exciter.coupling_curve_shape),
                "seed 0x{seed:08X}: coupling_curve {:.4} out of [0.85,1.15]",
                dna.exciter.coupling_curve_shape
            );
            assert!(
                (0.15..=0.35).contains(&dna.exciter.warmth_floor),
                "seed 0x{seed:08X}: warmth_floor {:.4} out of [0.15,0.35]",
                dna.exciter.warmth_floor
            );

            // Vibrator ranges
            assert!(
                (0.0..1.0).contains(&dna.vibrator.delay_micro_offset),
                "seed 0x{seed:08X}: delay_micro {:.4} out of [0,1)",
                dna.vibrator.delay_micro_offset
            );
            assert!(
                (0.95..=1.05).contains(&dna.vibrator.dispersion_asymmetry),
                "seed 0x{seed:08X}: dispersion_asym {:.5} out of [0.95,1.05]",
                dna.vibrator.dispersion_asymmetry
            );

            // Resonator ranges
            assert!(
                (0.997..=1.003).contains(&dna.resonator.modal_drift),
                "seed 0x{seed:08X}: modal_drift {:.6} out of [0.997,1.003]",
                dna.resonator.modal_drift
            );
            assert!(
                (0.05..=0.15).contains(&dna.resonator.stereo_offset),
                "seed 0x{seed:08X}: stereo_offset {:.4} out of [0.05,0.15]",
                dna.resonator.stereo_offset
            );
            ...
            assert!(
                (0.4..=1.2).contains(&mod_hz),
                "seed 0x{seed:08X}: mod_rate_hz {mod_hz:.4} out of [0.4,1.2]"
            );

            // Spatial ranges
            assert!(
                (0.55..=0.70).contains(&dna.spatial.reverb_diffusion),
                "seed 0x{seed:08X}: reverb_diff {:.4} out of [0.55,0.70]",
                dna.spatial.reverb_diffusion
            );
            assert!(
                (0.70..=0.90).contains(&dna.spatial.reverb_brightness),
                "seed 0x{seed:08X}: reverb_bright {:.4} out of [0.70,0.90]",
                dna.spatial.reverb_brightness
            );

            // Non-lin ranges
            assert!(
                (0.93..=1.07).contains(&dna.non_lin.saturation_asymmetry),
                "seed 0x{seed:08X}: sat_asym {:.4} out of [0.93,1.07]",
                dna.non_lin.saturation_asymmetry
            );
            assert!(
                (0.95..=1.05).contains(&dna.non_lin.transfer_inflection),
                "seed 0x{seed:08X}: inflection {:.4} out of [0.95,1.05]",
                dna.non_lin.transfer_inflection
            );
        }
    }
    ...
    fn sample_rate_affects_only_rate_params() {
        ...
        assert_eq!(
            at_44k.spatial.reverb_diffusion,
            at_96k.spatial.reverb_diffusion
        );

        // Rate-dependent params must differ
        assert_ne!(
            at_44k.resonator.modulation_rate_hz,
            at_96k.resonator.modulation_rate_hz
        );
    ...
    fn distribution_uniformity() {
        ...
        let sigs: Vec<f32> = (0..n)
            .map(|i| {
                InstrumentDna::from_seed(i, TEST_SAMPLE_RATE)
                    .exciter
                    .signature
            })
            .collect();
    ...
    fn display_format() {
        ...
        assert!(
            s.contains("spatial:"),
            "Display should include spatial section"
        );
    }
