...
use crate::dsp_core::{soft_saturate, DcBlocker, OnePole};
    ...
    pub fn process(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        for i in 0..input.len() {
            ...
            let tape_out = if tape_amt > 0.001 {
                ...
                let with_hyst = saturated * (1.0 - hyst_amt) + self.hysteresis_state * hyst_amt;
...
mod tests {
    ...
    fn output_bounded_under_strong_input() {
        ...
        assert!(
            p < 5.0,
            "Even at max everything, output should be bounded: peak={p}"
        );
    }
