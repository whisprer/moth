...
impl SpatialCharacter {
    pub const DRY: Self = Self { size: 0.0, brightness: 0.5, mix: 0.0 };
    pub const SMALL_ROOM: Self = Self { size: 0.3, brightness: 0.45, mix: 0.20 };
    pub const MEDIUM_ROOM: Self = Self { size: 0.5, brightness: 0.50, mix: 0.25 };
    pub const LARGE_HALL: Self = Self { size: 0.8, brightness: 0.40, mix: 0.30 };
    pub const CATHEDRAL: Self = Self { size: 0.95, brightness: 0.35, mix: 0.35 };
...
impl Default for SpatialCharacter {
    fn default() -> Self { Self::SMALL_ROOM }
}
...
impl SpatialProcessor {
    ...
    pub fn process(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        for i in 0..input.len() {
            ...
            self.delays[0].buffer[(self.delays[0].write_pos + FDN_DELAY_BUF - 1) % FDN_DELAY_BUF] = f0;
            self.delays[1].buffer[(self.delays[1].write_pos + FDN_DELAY_BUF - 1) % FDN_DELAY_BUF] = f1;
            self.delays[2].buffer[(self.delays[2].write_pos + FDN_DELAY_BUF - 1) % FDN_DELAY_BUF] = f2;
            self.delays[3].buffer[(self.delays[3].write_pos + FDN_DELAY_BUF - 1) % FDN_DELAY_BUF] = f3;
    ...
    pub fn reset(&mut self) {
        ...
        for d in self.damping.iter_mut() { d.reset(); }
