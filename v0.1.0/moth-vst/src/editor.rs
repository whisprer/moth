...
fn pfill(c: vg::Color) -> vg::Paint { vg::Paint::color(c) }
fn pstroke(c: vg::Color, w: f32) -> vg::Paint { let mut p = vg::Paint::color(c); p.set_line_width(w); p }
...
struct ExciterVis { params: Arc<MothParams> }
impl ExciterVis {
...
impl View for ExciterVis {
    fn element(&self) -> Option<&'static str> { Some("exciter-vis") }
    ...
    fn draw(&self, cx: &mut DrawContext, canvas: &mut Canvas) {
        ...
        if stoch > 0.01 {
            ...
            for &(px, py) in &top_pts[1..] { band_path.line_to(px, py); }
            for &(px, py) in bot_pts.iter().rev() { band_path.line_to(px, py); }
            band_path.close();
...
struct VibratorVis { params: Arc<MothParams> }
impl VibratorVis {
    pub fn new(cx: &mut Context, params: Arc<MothParams>) -> Handle<Self> {
        Self { params }.build(cx, |_| {})
    }
...
impl View for VibratorVis {
    fn element(&self) -> Option<&'static str> { Some("vibrator-vis") }
    ...
    fn draw(&self, cx: &mut DrawContext, canvas: &mut Canvas) {
        ...
        // Show as a decaying ceiling line across the partials
        {
            ...
            for p in 0..=partials {
                ...
                let rx = x + pad + (pf / (partials as f32 + 1.0)) * ew;
                let ry = mid_y + atten * (h * 0.38);
                if p == 0 { rolloff_m.move_to(rx, ry); } else { rolloff_m.line_to(rx, ry); }
            }
...
struct BodyVis { params: Arc<MothParams> }
impl BodyVis {
    pub fn new(cx: &mut Context, params: Arc<MothParams>) -> Handle<Self> {
        Self { params }.build(cx, |_| {})
    }
...
impl View for BodyVis {
    fn element(&self) -> Option<&'static str> { Some("body-vis") }
    ...
    fn draw(&self, cx: &mut DrawContext, canvas: &mut Canvas) {
        ...
        if stiff < -0.5 {
            ...
            body_path.bezier_to(cx2 + bw * 0.7, cy - bh, cx2 + bw, cy - bh * 0.5, cx2 + bw * 0.85, cy - bh * 0.15);
            body_path.bezier_to(cx2 + bw * waist, cy, cx2 + bw * waist, cy, cx2 + bw * 0.9, cy + bh * 0.2);
            body_path.bezier_to(cx2 + bw * 1.1, cy + bh * 0.6, cx2 + bw * 0.8, cy + bh, cx2, cy + bh);
            body_path.bezier_to(cx2 - bw * 0.8, cy + bh, cx2 - bw * 1.1, cy + bh * 0.6, cx2 - bw * 0.9, cy + bh * 0.2);
            body_path.bezier_to(cx2 - bw * waist, cy, cx2 - bw * waist, cy, cx2 - bw * 0.85, cy - bh * 0.15);
            body_path.bezier_to(cx2 - bw, cy - bh * 0.5, cx2 - bw * 0.7, cy - bh, cx2, cy - bh);
            body_path.close();
        } else if stiff < 0.7 {
            let bw = 32.0 * sc + 8.0;
            let bh = 22.0 * sc + 8.0;
        ...
        for i in 0..modes {
            ...
            let boosted_gain = if i == 0 { gain * 1.3 } else if i < 3 { gain * 1.15 } else { gain };
        ...
        // ── Purple: brightness rolloff envelope curve ──
        {
            ...
            for i in 0..=modes {
                ...
                if i == 0 { env.move_to(mx, my); } else { env.line_to(mx, my); }
            }
...
struct NonlinVis { params: Arc<MothParams> }
impl NonlinVis {
    pub fn new(cx: &mut Context, params: Arc<MothParams>) -> Handle<Self> {
        Self { params }.build(cx, |_| {})
    }
...
impl View for NonlinVis {
    fn element(&self) -> Option<&'static str> { Some("nonlin-vis") }
    ...
    fn draw(&self, cx: &mut DrawContext, canvas: &mut Canvas) {
        ...
        if warmth > 0.01 {
            ...
            wp.bezier_to(
                gc_x + warmth_w * 0.3, gc_y + gh * 0.5 - warmth_h,
                gc_x + warmth_w * 0.7, gc_y + gh * 0.5 - warmth_h * 0.8,
                gc_x + warmth_w, gc_y + gh * 0.5,
            );
            ...
            wpm.bezier_to(
                gc_x + gw - warmth_w * 0.3, gc_y + gh * 0.5 + warmth_h,
                gc_x + gw - warmth_w * 0.7, gc_y + gh * 0.5 + warmth_h * 0.8,
                gc_x + gw - warmth_w, gc_y + gh * 0.5,
            );
        ...
        if tone < 0.99 {
            ...
            tf.bezier_to(
                gc_x + gw - (gw - fade_start) * 0.5, gc_y + gh * rolloff_strength * 0.3,
                fade_start + (gw + gc_x - fade_start) * 0.2, gc_y,
                fade_start, gc_y,
            );
            tf.close();
            ...
            bf.bezier_to(
                gc_x + (fade_end - gc_x) * 0.5, gc_y + gh - gh * rolloff_strength * 0.3,
                fade_end - (fade_end - gc_x) * 0.2, gc_y + gh,
                fade_end, gc_y + gh,
            );
            bf.close();
        ...
        // ── Amber: ghost curve at higher drive ──
        {
            ...
            for i in 0..=steps {
                ...
                let out = if tt > 0.5 {
                    ...
                    prev = v; v
                } else if driven >= 0.0 { soft_sat(driven) }
                else { soft_sat(driven * asym) / asym };
...
struct SpatialVis { params: Arc<MothParams> }
impl SpatialVis {
    pub fn new(cx: &mut Context, params: Arc<MothParams>) -> Handle<Self> {
        Self { params }.build(cx, |_| {})
    }
...
impl View for SpatialVis {
    fn element(&self) -> Option<&'static str> { Some("spatial-vis") }
