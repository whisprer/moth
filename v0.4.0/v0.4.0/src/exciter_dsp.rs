...
use crate::dsp_core::{soft_saturate, DcBlocker, DspRng, OnePole};
...
impl ExciterProcessor {
    ...
    fn process_friction(
        ...
        let raw =
            self.friction.particle + (self.rng.next_bipolar() * 0.5 - self.friction.particle) * v4;
...
mod tests {
    ...
    fn soft_tilt_darker_than_hard() {
        ...
        let hf_soft: f32 = buf_soft.windows(2).map(|w| (w[1] - w[0]).abs()).sum();
        let hf_hard: f32 = buf_hard.windows(2).map(|w| (w[1] - w[0]).abs()).sum();
