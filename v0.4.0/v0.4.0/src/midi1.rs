...
///
/// ```
/// use moth::midi1::Midi1Normaliser;
///
            ...
            ParserState::Idle => {
                ...
                if let Some(status) = self.running_status {
                    ...
                    if msg_type == STATUS_CHAN_AT || msg_type == 0xC0 {
                        ...
                        self.parser = ParserState::WaitData2 {
                            status,
                            data1: byte,
                        };
                        return None;
...
mod tests {
    ...
    fn poly_aftertouch_updates_force() {
        ...
        assert!(
            at_force > initial_force,
            "Aftertouch should override velocity"
        );
    }
    ...
    fn poly_aftertouch_ignores_inactive_notes() {
        ...
        assert_eq!(
            ...
            "AT on inactive note should not change force"
        );
    }
    ...
    fn system_common_cancels_running_status() {
        ...
        assert_eq!(
            event, None,
            "Running status should be cancelled by System Common"
        );
    }
